...
// --- [Helper] 팀 정렬 로직 (안전하게 분리) ---
const getSortedTeams = (teams: MasterTeam[], tab: string, tier: string, region: string, search: string) => {
...
export default function FootballLeagueApp() {
  const [currentView, setCurrentView] = useState<'RANKING' | 'SCHEDULE' | 'HISTORY' | 'ADMIN' | 'TEAMS' | 'TUTORIAL'>('RANKING');
  ...
  // 🔥 Safe Filtering for Team Assignment
  const stepTeams = getSortedTeams(
    masterTeams.filter(mt => !(recordActiveS?.teams || []).map(t => t.name).includes(mt.name)),
  ...
  const handleConfirmTeam = async () => {
    ...
    if(o && m) {
      const nt: Team = { 
        id:Date.now(), seasonId:Number(adminTab), name:m.name, logo:m.logo||'', 
        ownerName:o.nickname, region: m.region, tier:m.tier||'A',
  ...
  // 🔥 [Fix] Auth Logic with Session & Expiry (3 Hours)
  const handleAdminAccess = () => {
    const storedAuth = sessionStorage.getItem('ef_admin_auth');
    const storedTime = sessionStorage.getItem('ef_auth_time');
    const threeHours = 3 * 60 * 60 * 1000;

    if (storedAuth === 'true' && storedTime && (Date.now() - Number(storedTime) < threeHours)) {
  ...
  // 🔥 [Safe] Variables (No useMemo here to avoid syntax issues)
  const filteredTeams = getSortedTeams(masterTeams, manageTab, manageTier, manageRegion, manageSearch);
    ...
    <div className="min-h-screen bg-[#020617] text-white font-black italic tracking-tighter overflow-x-hidden pb-20">
      ...
      >
        ...
        <div className="absolute bottom-6 left-6 uppercase z-20 pointer-events-none">
          ...
          <p className="text-emerald-400 text-[10px] md:text-xs font-sans not-italic tracking-widest mt-1">ver. League Master P_54</p>
        ...
        {currentView === 'ADMIN' && (
          <div className="animate-in fade-in space-y-10">
            ...
            {adminTab === 'TEAMS' && (
              ...
              <div className="bg-slate-900/60 p-8 rounded-3xl border border-slate-800 space-y-6"><div className="grid grid-cols-1 md:grid-cols-4 gap-3 font-sans not-italic"><select value={manageTab} onChange={e => setManageTab(e.target.value as any)} className="bg-slate-950 p-3 rounded border border-slate-700 text-xs"><option value="ALL">전체 팀 보기</option><option value="CLUB">클럽</option><option value="NATIONAL">국가대표</option></select><select value={manageTier} onChange={e => setManageTier(e.target.value)} className="bg-slate-950 p-3 rounded border border-slate-700 text-xs"><option value="ALL">전체 등급</option>{['S','A','B','C'].map(t=><option key={t} value={t}>{t}등급</option>)}</select><select value={manageRegion} onChange={e => setManageRegion(e.target.value)} className="bg-slate-950 p-3 rounded border border-slate-700 text-xs"><option value="ALL">리그/지역</option>{Array.from(new Set((manageTab==='ALL'?masterTeams:masterTeams.filter(t=>t.category===manageTab)).map(t=>t.region))).sort().map(r=><option key={r} value={r}>{r}</option>)}</select><input value={manageSearch} onChange={e => setManageSearch(e.target.value)} placeholder="팀을 검색해보세요" className="bg-slate-950 p-3 rounded border border-slate-700 text-xs" /></div><div className="grid grid-cols-2 md:grid-cols-6 gap-4">{getSortedTeams(masterTeams, manageTab, manageTier, manageRegion, manageSearch).slice(0, visibleTeamCount).map(mt => (<div key={mt.id} onClick={() => {setEditTeamId(mt.id!); setManualTeam(mt); manualFormRef.current?.scrollIntoView({behavior:'smooth'})}} className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex flex-col items-center gap-2 cursor-pointer hover:border-blue-500 transition-all relative group"><img src={mt.logo} alt="team" className="w-10 h-10 object-contain bg-white rounded-full p-1" /><p className="text-[10px] font-bold truncate w-full text-center">{mt.name}</p><p className="text-[9px] text-slate-500 truncate w-full text-center">{mt.region} • {mt.tier}등급</p><button onClick={(e) => {e.stopPropagation(); handleDeleteMasterTeam(mt.id!);}} className="absolute top-2 right-2 text-red-500 font-bold opacity-0 group-hover:opacity-100">×</button></div>))}</div>{visibleTeamCount < getSortedTeams(masterTeams, manageTab, manageTier, manageRegion, manageSearch).length && (<button onClick={() => setVisibleTeamCount(prev => prev + 18)} className="w-full py-3 bg-slate-800 text-slate-400 font-bold text-xs rounded-xl hover:bg-slate-700 transition-colors">👇 더 많은 팀 보기</button>)}</div>
              
              <section ref={manualFormRef} className="p-8 rounded-3xl border bg-slate-900/60 border-slate-800"><h3 className="text-xl mb-4 font-bold">{editTeamId ? '팀 수정하기' : '새로운 팀 등록'}</h3><div className="grid grid-cols-1 md:grid-cols-5 gap-4 font-sans not-italic mb-4"><select value={manualTeam.category} onChange={e => setManualTeam({...manualTeam, category: e.target.value as any})} className="bg-slate-950 p-3 rounded border border-slate-700 text-sm"><option value="CLUB">클럽</option><option value="NATIONAL">국가대표</option></select><select value={manualTeam.tier} onChange={e => setManualTeam({...manualTeam, tier: e.target.value as any})} className="bg-slate-950 p-3 rounded border border-slate-700 text-sm"><option value="S">S등급</option><option value="A">A등급</option><option value="B">B등급</option><option value="C">C등급</option></select>
              {/* 🔥 [Fix] Region Select Box */}
              <select value={manualTeam.region} onChange={e => setManualTeam({...manualTeam, region: e.target.value})} className="bg-slate-950 p-3 rounded border border-slate-700 text-sm"><option value="">지역/리그 선택</option>{REGION_OPTIONS.map(r => <option key={r} value={r}>{r}</option>)}</select>
              <input value={manualTeam.name} onChange={e => setManualTeam({...manualTeam, name: e.target.value})} placeholder="팀 이름" className="bg-slate-950 p-3 rounded border border-slate-700 text-sm" /><input value={manualTeam.logo} onChange={e => setManualTeam({...manualTeam, logo: e.target.value})} placeholder="로고 URL" className="bg-slate-950 p-3 rounded border border-slate-700 text-sm" /></div><button onClick={handleSaveMaster} className="w-full bg-emerald-600 py-3 rounded font-bold">{editTeamId ? '수정 저장하기' : '등록하기'}</button></section><section className="bg-slate-900/60 p-8 rounded-3xl border border-orange-500/30"><h3 className="text-orange-400 font-bold mb-4">한번에 등록하기</h3><textarea value={bulkInput} onChange={e => setBulkInput(e.target.value)} className="w-full h-24 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs mb-4" /><button onClick={handleBulk} className="w-full bg-orange-600 py-3 rounded font-bold">등록하기</button></section>
              </>
            )}
          </div>
        )}
